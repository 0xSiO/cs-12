...
@ string. Will convert all characters to lowercase, and ignore characters after
...
loop:
        ...
        orr     r3, r3, 0x20    @ else, convert char to lowercase with bit mask
        strb    r3, [r4]        @ store the lowercase char
        add     r4, r4, 1       @ increment pointer

        add     r5, r5, 1       @ increase length
        cmp     r5, r6          @ if length >= max length...
        bge     break           @   break

        mov     r0, STDIN       @ else, switch to keyboard input
        mov     r1, r4          @ specify where to put the char
